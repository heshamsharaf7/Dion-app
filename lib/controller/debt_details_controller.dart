...
class DebtDetailsController extends GetxController {
  ...
  DebtDetailsController({required this.storeCustomersRepo});
  StoreGetModel? store;
                ...
                GetBuilder<DebtDetailsController>(
                  builder: (context) {
                    if (invoiceItemsByInvoiceId == []) {
                      ...
                      var controller = Get.find<DebtDetailsController>();
                      return Column(
  ...
  getAllStoreTransactions() async {
    int storeId =
        Get.find<SharedPreferences>().getInt(AppConstants.CURRENT_STORE_ID)!;
    Response response = await storeCustomersRepo
        .getAllStoreCustomerTransactions(storeCustomer!.id, storeId);
    if (response.statusCode == 200) {
      transactions = [];
      response.body.forEach((o) {
